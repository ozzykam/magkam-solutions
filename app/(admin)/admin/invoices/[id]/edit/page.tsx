...
export default function EditInvoicePage() {
  ...
  const loadInvoice = async (id: string) => {
    try {
      ...
      if (data.discount) {
        ...
        setDiscountReason(data.discount.reason || '');
      }

      // Dates
      const issueDateObj = data.issueDate.toDate ? data.issueDate.toDate() : new Date(data.issueDate.toMillis());
      setIssueDate(issueDateObj.toISOString().split('T')[0]);

      const dueDateObj = data.dueDate.toDate ? data.dueDate.toDate() : new Date(data.dueDate.toMillis());
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user || !invoice) {
      alert('You must be logged in to edit an invoice');
      return;
    }
    ...
    try {
      ...
      // Only add optional fields if they have values, use null to remove
      if (title) {
  ...
  if (!invoice) {
    return (
      <div className="text-center py-12">
        ...
        <Button variant="primary" className="mt-4" onClick={() => router.push('/admin/invoices')}>
          Back to Invoices
        </Button>
      </div>
    );
