...
export default function NewProposalPage() {
  ...
  const handleSubmit = async (status: ProposalStatus.DRAFT | ProposalStatus.SENT) => {
    // Validation
    ...
    try {
      ...
      // Add clientId if customer was selected (not a new customer)
      if (clientId) {
      ...
      if (enableDiscount) {
        proposalData.discount = { type: discountType, value: discountValue, reason: discountReason };
      }
      if (title) {
      ...
      if (terms) {
        proposalData.terms = terms;
      }
      if (notes) {
        proposalData.notes = notes;
      }
      if (validUntil) {
        proposalData.validUntil = Timestamp.fromDate(new Date(validUntil));
      }
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-1">Create a new proposal for a client</p>
      </div>
      ...
      <Card>
        <div className="p-6 space-y-4">
          ...
          <CustomerSelector
            onSelectCustomer={(customerId, customerData) => {
      ...
      <div className="flex gap-3 justify-end pb-8">
        ...
        <Button variant="secondary" onClick={() => handleSubmit(ProposalStatus.DRAFT)} loading={saving}>
          Save as Draft
        </Button>
        <Button variant="primary" onClick={() => handleSubmit(ProposalStatus.SENT)} loading={saving}>
          Create & Send
        </Button>
      </div>
    </div>
